...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['AVARA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AVARA_LOG'] or 'warn' if it isn't set.
   */
...
export class Avara {
  ...
  constructor({
    ...
    if (apiKey === undefined) {
      throw new Errors.AvaraError(
        "The AVARA_API_KEY environment variable is missing or empty; either provide it, or instantiate the Avara client with an apiKey option, like new Avara({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Avara.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AVARA_LOG'), "process.env['AVARA_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('AVARA_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static AvaraError = Errors.AvaraError;
